...
def main():
    ...
    # Quick sanity check
    #
    # IMPORTANT: the runtime decision (TemporalAggregator._decide) compares
    # the RAW XceptionNet EMA directly to FAKE_RAW_THR — temperature scaling
    # is applied ONLY to smoothed_score for display, never before the
    # decision gate. A previous version of this check applied the
    # temperature-scaled score here instead, which does not match what the
    # classifier actually does and gave misleading FP/FN numbers. Compare
    # raw scores to fake_thr, exactly like the real decision path.
    fp = (real_scores >= info['fake_thr']).mean()
    fn = (fake_scores <  info['fake_thr']).mean()
    print(f"\nSanity check on your data (per-frame, raw-probability space — "
          f"matches the actual runtime decision gate):")
    print(f"  False positives (real flagged FAKE) : {fp*100:.1f}%")
    print(f"  False negatives (fake missed)       : {fn*100:.1f}%")
    print(f"  NOTE: this is a per-frame estimate only. The live system also "
          f"applies temporal aggregation (EMA, sustained-frame counts, "
          f"hysteresis, a noise guard) on top of this per-frame gate, which "
          f"changes the effective video-level FPR/FNR. Do not quote these "
          f"numbers as a guaranteed video-level rate.")
