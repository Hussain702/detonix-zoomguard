...
def process_video(video_path, output_path, config, show_display=True):
    ...
    if not cap.isOpened():
        logger.error(f"Cannot open: {video_path}"); return None
...
def main():
    ...
    if args.video:
        ...
        inp = Path(args.input)
        if not inp.exists():
            print(f"\n  [ERROR] '{inp}' not found. Create it and drop videos in.\n")
            sys.exit(1)
    ...
    print(f"\n  Found {len(videos)} video(s)  |  threshold: {args.threshold}  |  skip: {args.skip}")
    if args.dashboard:
    ...
    for vp in videos:
        ...
        if res: all_results[vp.name] = res

    fakes = sum(sum(1 for i in r.values() if i.get('is_deepfake')) for r in all_results.values())
    print(f"\n  Done  |  {len(all_results)} video(s)  |  {fakes} deepfake track(s) flagged")
    print(f"  Output: {args.output}/   Logs: logs/\n")
